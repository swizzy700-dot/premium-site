...
export class AIService {
    ...
    async analyzeAudit(url, scores, pageTitle) {
...
Return ONLY a JSON object in this exact format:
{
  ...
  "weaknesses": ["List of critical issues"]
            ...
            logger.error('AI analysis failed', { url, error: error.message });
            // Return mock analysis on error
            return this.getMockAnalysis(scores);
        }
    }
    buildPrompt(url, scores, pageTitle) {
...
- 0-49: Critical, major overhaul needed`;
    ...
    getMockAnalysis(scores) {
        ...
        if (scores.performance >= 80)
            ...
            weaknesses.push('Slow performance affecting user experience');
        if (scores.seo >= 80)
            strengths.push('Good SEO foundation');
        else
            weaknesses.push('SEO issues limiting search visibility');
        if (scores.accessibility >= 80)
            strengths.push('Accessible design');
        else
            weaknesses.push('Accessibility barriers for some users');
        if (scores.bestPractices >= 80)
            strengths.push('Follows web best practices');
        else
            weaknesses.push('Security or best practice concerns');
        return {
            insights,
