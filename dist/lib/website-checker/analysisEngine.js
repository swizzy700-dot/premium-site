 ...
 */
export function analyzeBusinessImpact(audit, url) {
 ...
 */
function generateExecutiveSummary(primary, secondary, url) {
    const scores = primary.scores;
    const worstScore = Math.min(scores.performance.score, scores.seo.score, scores.accessibility.score, scores["best-practices"].score);
 ...
 */
function generatePerformanceAnalysis(primary, secondary) {
    return {
 ...
 */
function analyzeSpeed(score, metrics) {
 ...
 */
function analyzeAccessibility(score) {
    return {
        score: score.score,
        grade: score.label,
        status: getStatusFromScore(score.score),
 ...
 */
function analyzeBestPractices(score) {
    return {
        score: score.score,
        grade: score.label,
        status: getStatusFromScore(score.score),
 ...
 */
function analyzeMobile(primary, secondary) {
 ...
 */
function calculateBusinessImpact(primary, secondary) {
    const scores = primary.scores;
    ...
    else if (perf < 75 || seo < 75)
        overallRisk = "high";
    else if (perf < 90 || seo < 90)
    ...
    return {
        ...
        mobileRevenueRisk: assessMobileRevenueRisk(primary.scores.performance.score, secondary?.scores.performance.score),
 ...
 */
function generateFixRoadmap(primary) {
    const quickWins = [];
    const shortTerm = [];
    const strategic = [];
 ...
 */
function generateCompetitiveContext(primary, secondary) {
...
function countCriticalIssues(audit) {
    let count = 0;
    Object.values(audit.scores).forEach((score) => {
        if (score.score < 50)
            count++;
    });
    if (audit.metrics?.lcp?.severity === "poor")
        count++;
    if (audit.metrics?.cls?.severity === "poor")
        count++;
    return count;
}
function countOpportunities(audit) {
    let count = 0;
    Object.values(audit.scores).forEach((score) => {
        if (score.score < 90)
            count++;
...
function calculateEstimatedImpact(scores) {
    const min = Math.min(scores.performance.score, scores.seo.score, scores.accessibility.score, scores["best-practices"].score);
    if (min < 50)
        return "Potential 25-40% improvement in conversions";
    if (min < 75)
        return "Potential 15-25% improvement in conversions";
    if (min < 90)
        return "Potential 5-15% improvement in conversions";
    return "Maintain current performance to preserve advantage";
}
function getSpeedBusinessMeaning(score) {
    if (score >= 90)
        return "Your website loads instantly, keeping users engaged";
    if (score >= 75)
        return "Good speed, but faster sites capture more market share";
    if (score >= 50)
        return "Slow loading is causing visitors to leave before converting";
    return "Critical speed issues are severely impacting your business";
}
function getSpeedUserImpact(score) {
    if (score >= 90)
        return "Users can interact immediately - zero friction";
    if (score >= 75)
        return "Minor delays may cause some impatience";
    if (score >= 50)
        return "Users experience noticeable frustration waiting for content";
    return "Most users abandon before the page finishes loading";
}
function getSpeedRevenueImpact(score) {
    if (score >= 90)
        return "Maximized conversion potential - every millisecond optimized";
    if (score >= 75)
        return "Losing 5-10% of potential revenue to slow load times";
    if (score >= 50)
        return "Losing 20-30% of potential revenue to poor performance";
    return "Critical revenue loss - 40%+ of mobile users never see your offer";
}
function getSEOBusinessMeaning(score) {
    if (score >= 90)
        return "Excellent search visibility - capturing maximum organic traffic";
    if (score >= 75)
        return "Good SEO foundation with room to outrank competitors";
    if (score >= 50)
        return "SEO gaps limiting organic reach and discoverability";
    return "Critical SEO issues preventing search engine visibility";
}
function getSEOUserImpact(score) {
    if (score >= 90)
        return "Users find you easily through search";
    if (score >= 75)
        return "Decent visibility but missing opportunities";
    if (score >= 50)
        return "Harder to find = fewer qualified visitors";
    return "Essentially invisible to search engines";
}
function getSEORevenueImpact(score) {
    if (score >= 90)
        return "Capturing maximum qualified organic traffic";
    if (score >= 75)
        return "Missing 20-30% of potential organic revenue";
    if (score >= 50)
        return "Losing 40-60% of search-driven revenue opportunities";
    return "Near-zero organic traffic = massive revenue loss";
}
function getAccessibilityBusinessMeaning(score) {
    if (score >= 90)
        return "Inclusive design expands your market reach";
    if (score >= 75)
        return "Good accessibility with minor barriers";
    if (score >= 50)
        return "Accessibility barriers excluding potential customers";
    return "Significant accessibility issues limiting audience";
}
function getAccessibilityUserImpact(score) {
    if (score >= 90)
        return "All users can access your services equally";
    if (score >= 75)
        return "Most users can navigate, some face challenges";
    if (score >= 50)
        return "Users with disabilities struggle to complete actions";
    return "Many users completely unable to use your site";
}
function getAccessibilityRevenueImpact(score) {
    if (score >= 90)
        return "Maximum market reach - no exclusions";
    if (score >= 75)
        return "Missing 10-15% of potential market (disabilities)";
    if (score >= 50)
        return "Excluding 20-25% of potential customers";
    return "Significant market exclusion - legal risk in many jurisdictions";
}
function getBestPracticesBusinessMeaning(score) {
    if (score >= 90)
        return "Industry-leading technical implementation";
    if (score >= 75)
        return "Solid technical foundation";
    if (score >= 50)
        return "Technical debt accumulating";
    return "Outdated practices causing security/reliability issues";
}
function getBestPracticesUserImpact(score) {
    if (score >= 90)
        return "Secure, reliable, modern experience";
    if (score >= 75)
        return "Generally secure with minor issues";
    if (score >= 50)
        return "Users may encounter security warnings";
    return "Users at risk - security vulnerabilities present";
}
function getBestPracticesRevenueImpact(score) {
    if (score >= 90)
        return "Maximum trust and credibility";
    if (score >= 75)
        return "Good trust signals";
    if (score >= 50)
        return "Trust erosion affecting conversions";
    return "Security issues directly impacting credibility and sales";
}
function assessConversionRisk(perfScore) {
    if (perfScore >= 90) {
        return {
            level: "low",
...
function assessSEOVisibilityRisk(seoScore) {
    if (seoScore >= 90) {
        return {
            level: "low",
...
function assessUXRisk(accessibilityScore, perfScore) {
...
function assessMobileRevenueRisk(mobileScore, desktopScore) {
    if (mobileScore >= 90) {
        return {
            level: "low",
...
function generateOpportunitySummary(riskLevel) {
    const summaries = {
...
function calculateTimeline(quickWins, shortTerm, strategic) {
    const weeks = Math.ceil((quickWins.length * 0.5 + shortTerm.length * 1 + strategic.length * 2) / 5);
    return weeks <= 1 ? "1 week" : `${weeks} weeks`;
}
function calculateInvestment(quickWins, shortTerm, strategic) {
    const hours = quickWins.reduce((sum, f) => sum + (f.effort === "small" ? 4 : 8), 0) +
...
function calculateROI(quickWins, shortTerm, strategic) {
    const totalItems = quickWins.length + shortTerm.length + strategic.length;
    if (totalItems === 0)
        return "N/A - No fixes required";
