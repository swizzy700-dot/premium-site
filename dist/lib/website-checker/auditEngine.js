 ...
 */
export async function runWebsiteAudit(url) {
    const scannedAt = new Date().toISOString();
    try {
        // Validate URL
        if (!isValidUrl(url)) {
            return createErrorResponse("Invalid URL provided", "validation_error", scannedAt);
        }
        ...
        if (!apiKey || typeof apiKey !== "string" || apiKey.trim() === "") {
            ...
            return createErrorResponse(ERROR_MESSAGES.config_error, "config_error", scannedAt);
        }
        ...
        return createErrorResponse(combinedError || ERROR_MESSAGES.scan_error, "scan_error", scannedAt);
    }
    catch (error) {
        // Ultimate safety net - should never reach here but just in case
        const errorMsg = error instanceof Error ? error.message : "Unknown error";
        auditLogger.crash("AuditEngine", error);
        return createErrorResponse(errorMsg || ERROR_MESSAGES.unknown, "unknown", scannedAt);
    }
}
/**
 * Combine error messages from both audits
 */
function combinedErrors(mobileError, desktopError) {
    if (!mobileError && !desktopError)
        return null;
    const errors = [];
    if (mobileError)
        errors.push(`Mobile: ${mobileError}`);
    if (desktopError)
        errors.push(`Desktop: ${desktopError}`);
    return errors.join(" | ");
}
/**
 ...
 */
async function fetchPageSpeedDataSafe(url, strategy, apiKey, timeoutMs, attempt = 0) {
    const MAX_RETRIES = 1;
    ...
    try {
        ...
        return { success: true, data: data.lighthouseResult, error: null };
    }
    catch (error) {
        clearTimeout(timeoutId);
 ...
 */
function processPageSpeedData(rawLhr) {
    ...
    const scores = {
        performance: createScoreDetail(Math.round((categories.performance?.score ?? 0) * 100), "performance"),
        seo: createScoreDetail(Math.round((categories.seo?.score ?? 0) * 100), "seo"),
        accessibility: createScoreDetail(Math.round((categories.accessibility?.score ?? 0) * 100), "accessibility"),
        "best-practices": createScoreDetail(Math.round((categories["best-practices"]?.score ?? 0) * 100), "best-practices"),
    };
 ...
 */
function createScoreDetail(score, category) {
 ...
 */
function getScoreExplanation(category, score, label) {
    const explanations = {
        performance: {
 ...
 */
function isValidPageSpeedResponse(data) {
    if (!data || typeof data !== "object")
        return false;
    const d = data;
    return (typeof d.lighthouseResult === "object" &&
        d.lighthouseResult !== null);
}
/**
 * Create standardized error response
 */
function createErrorResponse(message, errorType, scannedAt) {
    return {
        success: false,
        source: null,
        desktop: null,
 ...
 */
export function getWorstCategory(scores) {
    if (!scores)
        return null;
    const entries = Object.entries(scores);
    if (entries.length === 0)
        return null;
    return entries.reduce((worst, [key, detail]) => detail.score < scores[worst].score ? key : worst, entries[0][0]);
}
/**
