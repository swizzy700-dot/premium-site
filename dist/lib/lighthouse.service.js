...
export class LighthouseService {
    ...
    async runAudit(url) {
        ...
        if (mobileResult.status === 'rejected') {
            logger.error(`Mobile audit failed`, { url, error: mobileResult.reason.message });
        }
        if (desktopResult.status === 'rejected') {
            logger.error(`Desktop audit failed`, { url, error: desktopResult.reason.message });
        }
     ...
     */
    async runDeviceAudit(url, device, timeoutMs) {
        const startTime = Date.now();
        let chrome = null;
        try {
            ...
            throw new AuditError({
                message: `${device} audit failed: ${error.message}`,
     ...
     */
    createTimeoutPromise(ms, device) {
    ...
    async killChrome(chrome) {
        ...
        try {
            ...
            logger.debug('Chrome kill attempted (may already be dead)', { error: error.message });
    ...
    calculateOverallScore(result) {
        ...
        const overall = Math.round(performance * 0.4 +
            seo * 0.2 +
            accessibility * 0.25 +
            bestPractices * 0.15);
        // Calculate grade
