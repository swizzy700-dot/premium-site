import type { LighthouseAdminAudit, LighthouseClientAudit, LighthouseScoreKey } from "./types";
...
function friendlyAudit(params: { auditId: string; title: string | undefined; score: number | undefined; category: LighthouseScoreKey }) {
  const { auditId, title, score, category } = params;

  const baseTitle = title?.trim() ? title.trim() : auditId;

  const mapped: Record<string, { title: string; description: string; defaultSteps: string[] }> = {
...
function getMetric(params: {
  ...
  auditId: string;
  format: (audit: unknown) => { formatted: string; severity: "good" | "warning" | "bad" } | null;
}) {
...
export function interpretLighthouse(params: {
  ...
  const worstKey = (Object.keys(scores) as LighthouseScoreKey[]).sort((a, b) => scores[a] - scores[b])[0];

  const scoreCards = (Object.keys(scores) as LighthouseScoreKey[]).reduce((acc, key) => {
    ...
    return acc;
  }, {} as LighthouseClientAudit["scores"]);
  ...
  const lcp = getMetric({
    ...
    format: (audit) => {
      ...
      const formatted = typeof auditObj?.displayValue === "string" ? (auditObj.displayValue as string) : "";
      const numericValue =
        typeof auditObj?.numericValue === "number" ? (auditObj.numericValue as number) : undefined;
  ...
  const issuesFound: LighthouseClientAudit["issuesFound"] = [];
  const recommendations: LighthouseClientAudit["recommendations"] = [];

  const categoryKeys: LighthouseScoreKey[] = ["performance", "seo", "accessibility", "best-practices"];
  ...
  for (const key of categoryKeys) {
    ...
    for (const ref of sorted) {
      const audit = parsed.auditsById[ref.id] as unknown;
      const friendly = friendlyAudit({
        auditId: ref.id,
        title: typeof (audit as Record<string, unknown>)?.title === "string" ? ((audit as Record<string, unknown>).title as string) : undefined,
        score: typeof ref.score === "number" ? ref.score : undefined,
        category: key,
      });

      const severity: "info" | "warning" | "critical" =
        ref.score != null ? severityFromScore(ref.score) : baseSeverity;
  ...
  const client: LighthouseClientAudit = {
    url,
    businessName,
    generatedAt,
  ...
  const admin: LighthouseAdminAudit = { client, rawLhr };
  return admin;
}
