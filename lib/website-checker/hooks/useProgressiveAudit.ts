...
const createInitialState = (url: string): ProgressiveAuditState => ({
  url,
  phase: 'idle',
  mobile: null,
  desktop: null,
...
export function useProgressiveAudit(options: UseProgressiveAuditOptions) {
  const { url, businessName, onComplete, onError, onPartial } = options;
  
  // State
  const [state, setState] = useState<ProgressiveAuditState>(() => createInitialState(url));
  ...
  const updateState = useCallback((updater: (prev: ProgressiveAuditState) => ProgressiveAuditState) => {
    setState(prev => {
  ...
  const startAudit = useCallback(async () => {
    ...
    try {
      ...
      const phase: ProgressiveAuditState['phase'] = isFullyComplete 
        ? 'completed' 
      ...
      const newState: ProgressiveAuditState = {
        url,
        phase,
        mobile: mobileData,
        desktop: desktopData,
